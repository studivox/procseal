...
function makeLive(
  ...
  return {
    ...
    safeName: toSafeLabelOrRedacted('test-app'),
    pm2Id: 0,
    status: 'online',
...
test('no findings when every declared value matches the live value exactly', () => {
  ...
  const findings = evaluateRules({ declared, live, checkUnexpected: false });
...
test('PS003 fires only when checkUnexpected is explicitly true', () => {
  ...
  const findings = evaluateRules({ declared, live, checkUnexpected: true });
  assert.equal(findings.length, 1);
