...
- **PS004** (`src/rules/reuse.ts`, opt-in via `--check-reuse`): compares the selected process's PS004-eligible values (see [Candidate policy](#candidate-policy-for-ps004-check-reuse) above) against every _other PM2 application_ in the same run's snapshot — no second PM2 invocation. Reuse is defined by **application identity** (PM2's process name), not by process record: a PM2 cluster-mode application running several worker records all sharing one name still counts as exactly one other application, never one per worker, and a record belonging to the selected application's own name is never counted as "another" one, however many worker records it has. A process whose name failed safe-label validation is excluded from this grouping entirely — `[REDACTED]` is never treated as a shared application identity, a documented false-negative boundary. Eligibility is computed once at the PM2 adapter boundary, from the raw key/value, before the value is wrapped in an opaque `ObservedValue`, and stored only as a derived boolean (`Pm2EnvironmentVariable.reuseCandidate`) — never a raw length or the value itself; a key name that fails validation is never a candidate either. Findings are deduplicated (a value reused under two names within the selected process, or shared with three or more other applications, still produces exactly one finding) and returned in deterministic order, sorted by variable name.
