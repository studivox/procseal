...
- A read-only dotenv-file adapter (`src/adapters/dotenv-file.ts`) that reads exactly the file passed to `--env` — regular files only (symlinks and non-regular files rejected via `O_NOFOLLOW`, race-free), a 1 MiB size limit checked before and after reading, and per-file limits on variable count, key length, and value size (measured in UTF-8 bytes). Every declared value is registered in the run's `SecretRegistry` and wrapped in the same opaque `ObservedValue` the PM2 adapter uses, sharing one `Fingerprinter` per audit run so declared and live values compare through full-HMAC equality.
